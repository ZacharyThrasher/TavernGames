...
import { initPremiumEffects, teardownPremiumEffects, GoldOdometer } from "../ui/premium-fx.js";
...
export class TavernApp extends HandlebarsApplicationMixin(ApplicationV2) {
  ...
  async _prepareContext() {
    ...
    return {
      ...
      // V5.22+: Atmosphere and Flavor System
  ...
  _onRender(context, options) {
    ...
    // V5.26: Premium Effects Engine — Holographic tilt, gold dust trail,
    // crown jewels, table enchantment, streak auras, odometer
    teardownPremiumEffects(this.element);
    initPremiumEffects(this.element);

    // V5.26: Odometer — update pot display with tumbling digits
    const potAmountEl = this.element.querySelector(".pot-amount");
    if (potAmountEl && context.pot !== undefined) {
      GoldOdometer.update(potAmountEl, context.pot ?? 0, this._odometerInitialized ?? false);
      this._odometerInitialized = true;
    }
